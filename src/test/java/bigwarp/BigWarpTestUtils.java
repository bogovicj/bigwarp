...
import static bigwarp.url.UrlParseTest.TIFF_FILE_3D;
...
public class BigWarpTestUtils
{
	...
	public static String createTemp3DImage( String title, String format )
	{
		...
		try
		{
			ImagePlus img3d = NewImage.createByteImage( title, 8, 8, 4, NewImage.FILL_RAMP );
			tmpImg = Files.createTempFile( title, "." + format );
			tmpImg.toFile().delete();
			IJ.save( img3d, tmpImg.toFile().getCanonicalPath() );
			tmpImg.toFile().deleteOnExit();
			return tmpImg.toString();
		}
		catch ( Exception e )
		{
			if (tmpImg != null) {
	...
	public static String createTemp2DImage( String title, String format )
	{
		...
		try
		{
			ImagePlus img2d = NewImage.createByteImage( title, 8, 8, 1, NewImage.FILL_RAMP );
			tmpImg = Files.createTempFile( title, "." + format );
			tmpImg.toFile().delete();
			IJ.save( img2d, tmpImg.toFile().getCanonicalPath() );
			tmpImg.toFile().deleteOnExit();
			return tmpImg.toString();
		}
		catch ( Exception e )
	...
	static < T > BigWarp< T > createBigWarp( boolean[] moving ) throws SpimDataException, URISyntaxException, IOException
	{
		...
		final String tmpPath = createTemp3DImage( "img", "tif" );
